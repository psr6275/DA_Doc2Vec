...
from numpy import zeros,exp,outer, sum as np_sum, add as np_add, concatenate, \
...
def train_da_sg_pair(model,word, doctag_index, alpha, learn_vectors = True, learn_hidden = True,
                    context_vectors = None, context_locks = None, compute_loss = False):
    if context_vectors is None:
        context_vectors = model.docvecs.doctag_syn0
    if context_locks is None:
        context_locks = model.docvecs.doctag_syn0_lockf
    if word not in model.wv.vocab:
        return
    st_label = model.st_label[doctag_index]
    predict_word = model.wv.vocab[word]

    l1 = context_vectors[doctag_index]
    lock_factor = context_locks[doctag_index]

    neu1e = zeros(l1.shape)

    if model.hs:
        if st_label:
            l2a = deepcopy(model.wv.syn0_s[predict_word.point])
        else:
            l2a = deepcopy(model.wv.syn0_t[predict_word.point])
        prod_term = dot(l1,l2a.T)
        fa = expit(prod_term)
        ga = (1-predict_word.code-fa)*alpha
        if learn_hidden:
            if st_label:
                model.wv.syn0_s[predict_word.point] +=outer(ga,l1)
            else:
                model.wv.syn0_t[predict_word.point] +=outer(ga,l1)
        neu1e +=dot(ga,l2a)

        #loss component crresponding to hierarchical softmax
        if compute_loss:
            sgn = (-1.0)**predoct_word.code
            lprob = -log(expit(-sgn*pred_term))
    ...
    if model.negative:
        ...
        if st_label:
            l2b = model.wv.syn0_s[word_indices] 
        else:
            l2b = model.wv.syn0_t[word_indices] 
        ...
        if learn_hidden:
            if st_label:
                model.wv.syn0_s[word_indices] += outer(gb,l1)
            else:
                model.wv.syn0_t[word_indices] += outer(gb,l1)
    ...
    return neu1e
def train_batch_da_sg(model,sentences, doctag_indexes, alpha, work = None, compute_loss = False):
    result = 0
    st_label = model.st_label
    for doc_idx,sentence in enumerate(sentences):
        st_lab = st_label[doctag_indexes[doc_idx]]
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            for pos2, word2 in enumerate(word_vocabs[start:(pos+model.window+1-reduced_window)],start):
                if pos2 != pos:
                    if st_lab:
                        cc = train_sg_pair(model, model.wv.index2word[word.index], word2.index, alpha,context_vectors = model.wv.syn0_s,
                                context_locks = model.syn0_lockf_s, compute_loss = compute_loss)
                    else:
                        train_sg_pair(model, model.wv.index2word[word.index], word2.index, alpha,context_vectors = model.wv.syn0_t, 
                                context_locks = model.syn0_lockf_t, compute_loss = compute_loss)
...
def train_sg_pair(model,word, context_index,alpha, learn_vectors = True, learn_hidden = True, 
    ...
    if model.hs:
        ...
        ga = (1-predict_word.code-fa)*alpha
        if learn_hidden:
            model.syn1[predict_word.point]+=outer(ga,l1)
        neu1e += dot(ga,l2a)
        if compute_loss:
...
def train_document_da_dbow(model, doc_words, doctag_indexes, alpha, work=None,
                            train_words=False, learn_doctags=True, learn_words=True, learn_hidden=True,
                            word_vectors=None, word_locks=None, doctag_vectors=None, doctag_locks=None,
                            doctag_vectors2=None, doctag_locks2=None):
    """
    Update distributed bag of words model ("PV-DBOW") by training on a single document.
    Called internally from `Doc2Vec.train()` and `Doc2Vec.infer_vector()`.
    ...
    if doctag_vectors is None:
        doctag_vectors = model.docvecs.doctag_syn0
        doctag_vectors2 = model.docvecs.doctag_syn1
    if doctag_locks is None:
        doctag_locks = model.docvecs.doctag_syn0_lockf
        doctag_locks2 = model.docvecs.doctag_syn1_lockf
    if train_words and learn_words:
        # train nonshared document vectors( learn w0 for w1)
        train_batch_da_sg(model, [doc_words],doctag_indexes, alpha, work)
        #print(cc)
    #print(sum(model.syn1neg!=0))
    for doctag_index in doctag_indexes:
        for word in doc_words:
            # train nonshared document vectors( learn d0 for w0)
            train_da_sg_pair(model, word, doctag_index, alpha, learn_vectors=learn_doctags,
                              learn_hidden=learn_hidden, context_vectors=doctag_vectors,
                              context_locks=doctag_locks)
            # train shared document vectors( learn d1 for w1)
            train_sg_pair(model,word, doctag_index, alpha, learn_vectors=learn_doctags,
                                learn_hidden = learn_hidden, context_vectors = doctag_vectors2,
                                context_locks = doctag_locks2)
    return len(doc_words)
...
class DocvecsArray(utils.SaveLoad):
    ...
    def indexed_doctags(self, doctag_tokens):
        ...
        return ([self._int_index(index) for index in doctag_tokens if index in self],
                self.doctag_syn0, self.doctag_syn0_lockf, self.doctag_syn1, 
                self.doctag_syn1_lockf,doctag_tokens)
    ...
    def reset_weights(self, model):
        ...
        if self.mapfile_path:
            ...
            if model.st_idx:
                self.doctag_syn1 = empty((length, model.vector_size), dtype=REAL)
                model.wv.syn0_s = empty((vocab_length, model.vector_size), dtype=REAL)
                model.wv.syn0_t = empty((vocab_length, model.vector_size), dtype = REAL)
                self.doctag_syn1_lockf = ones((length,), dtype=REAL)
                model.syn0_lockf_s = ones((vocab_length,), dtype=REAL)
                model.syn0_lockf_t = ones((vocab_length,), dtype = REAL)
        if model.st_idx:
            for i in xrange(length):
                ...
                seed2 = "%s %d" % (self.index_to_doctag(i), model.seed)
                self.doctag_syn1[i] = model.seeded_vector(seed2)
            for i in xrange(vocab_length):
                model.wv.syn0_s[i] = model.seeded_vector(model.wv.index2word[i]+str(model.seed))
                model.wv.syn0_t[i] = model.seeded_vector(str(model.seed)+model.wv.index2word[i])
        else:
            for i in xrange(length):
                # construct deterministic seed from index AND model seed
                seed = "%d %s" % (model.seed, self.index_to_doctag(i))
...
class Doc2Vec(Word2Vec):
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for doc in job:
            ...
            doctag_indexes, doctag_vectors, doctag_locks,doctag_vectors2, doctag_locks2, ignored = indexed_doctags
            if self.st_label is not None:
                tally += train_document_da_dbow(self, doc.words, doctag_indexes, alpha, work,
                        train_words = self.dbow_words, doctag_vectors = doctag_vectors,
                        doctag_locks = doctag_locks, doctag_vectors2 = doctag_vectors2,
                        doctag_locks2 = doctag_locks2)
            elif self.sg:
