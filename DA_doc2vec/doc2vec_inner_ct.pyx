...
cdef unsigned long long fast_document_dbow_neg(
    ...
    return next_random
"""
cdef unsigned long long fast_document_dbow_ct_neg(
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len,
    REAL_t *context_vectors, REAL_t *syn1neg, const int size, const np.uint32_t word_index,
    const np.uint32_t context_index, const REAL_t alpha, REAL_t *work,
    unsigned long long next_random, int learn_context, int learn_hidden, REAL_t *context_locks) nogil:
...
def train_document_dbow_CT(model, doc_words, doctag_indexes, alpha, work=None,
    ...
    st_lab = model.st_label[doctag_indexes[0]]
    cdef int _st_label = st_lab #st_label is integer value if 1, source document. if 0, target document.
    ...
    cdef np.uint32_t *cum_table_docs
    ...
    if negative:
        ...
        if st_lab:
            cum_table_docs = <np.uint32_t *>(np.PyArray_DATA(model.cum_table_s))
        else:
            cum_table_docs = <np.uint32_t *>(np.PyArray_DATA(model.cum_table_t))
    ...
    with nogil:
        for i in range(document_len):
            ...
            for j in range(doctag_len):
                ...
                if negative:
                    next_random = fast_document_dbow_neg(negative, cum_table_docs, cum_table_len, _doctag_vectors, syn1neg, size,
